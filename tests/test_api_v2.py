...
def test_v2_create_requires_and_replays_idempotency_key(tmp_path):
    ...
    headers = {**auth, "Idempotency-Key": "create-request-0001"}
    first = client.post("/v2/sessions", headers=headers, json=body)
    second = client.post("/v2/sessions", headers=headers, json=body)
...
def test_v2_rejects_idempotency_key_reuse_with_different_payload(tmp_path):
    ...
    headers = {
        ...
        "Idempotency-Key": "create-request-0002",
    }
    first = client.post("/v2/sessions", headers=headers, json=create_body())
    assert first.status_code == 201

    changed = create_body()
    changed["theme"] = "different-theme"
    replay = client.post("/v2/sessions", headers=headers, json=changed)
