...
State loadはactive/supersededを含む全handoffのevidence partitionを各handoff cutoff以下のgeneration registryと再照合する。Global refはnull candidate、candidate refはmap keyと同一identityを必須とし、unknown、cross-candidate、future、duplicate、manifest union不整合を拒否する。
