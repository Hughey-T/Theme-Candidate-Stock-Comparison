...
6. 比較不能を点数化せず、異業態は期待return/損失確率/期間へ変換する。候補分類はPRIMARY/SECONDARY/CONDITIONAL/WATCH/EXCLUDEDだけとし、全体判断SELECTION/NO_SELECTIONと分離する。必ず勝者を作らない。
...
10. Custom GPTがphase固有artifactを生成し、runtimeがSchema・semantic整合性を検証保存する。runtimeが市場調査や分析文章を生成したと表現しない。
