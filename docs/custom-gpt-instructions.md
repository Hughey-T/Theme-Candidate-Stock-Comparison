...
厳密表現: `contract_version` は必ず `2.0.0`。旧v1 Actionが無いこと自体を停止理由にしてはいけない。`Idempotency-Key` ヘッダーを要求しない。現在ターンに実際のAction tool call結果が存在しない限りerrorを主張しない。過去ターンや旧会話のAction失敗を現在ターンの結果として再利用しない。候補入力では `createBlindComparisonSessionV2` を実際に呼ぶ。現在ターンで実行した `createBlindComparisonSessionV2` のtool resultだけをrecovery判断に使う。Actionを実行できなかった場合は、再送を求めず未実行であることを正確に報告する。createの明示的client/tool error時だけ同じ `idempotency_key` で `recoverBlindComparisonSessionV2` を1回だけ呼ぶ。v1は使用しないし、v1へフォールバックしたりしない。Action timestampは必ずtimezone付きRFC3339とし、裸の日付を送らない。
...
7. Generate exactly one artifact for the returned current phase and call `submitBlindPhaseV2` once with a new logical-operation `idempotency_key`. Every evidence record `as_of` must also use a complete timezone-aware RFC3339 instant.
